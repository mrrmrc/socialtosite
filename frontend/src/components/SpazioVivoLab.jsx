...
export function SpazioVivoLab({ token }) {
  ...
  useEffect(() => { apiFetch('/api/index.php?action=admin-spazio-vivo-lab', {}, token).then(setData).catch(e => setError(e.message)); }, [token]);
  ...
  return <section className={`sv-lab ${fullscreen ? 'is-fullscreen' : ''}`}>
    <header className="sv-lab-head"><div><span>LAB PRIVATO · SOLO ADMIN · CAVIA: SANGERMANO</span><h2>10 modi di entrare in uno Spazio Vivo</h2><p>Non template di siti web, ma dieci architetture di fruizione. Provale e valuta come cambia la scoperta dei contenuti.</p></div><div><strong>{posts.length}</strong><span>contenuti reali<br />in prova</span></div></header>
    ...
    <div className="sv-decision"><div><span>PROPOSTA {concept.n}</span><strong>{concept.name}</strong><p>{concept.line}</p></div><dl><div><dt>Ideale per</dt><dd>{concept.best}</dd></div><div><dt>Attenzione</dt><dd>{concept.risk}</dd></div></dl><section>{['Scoperta', 'Impatto', 'Orientamento'].map((label, i) => <div key={label}><span>{label}</span><i>{[0, 1, 2, 3, 4].map(n => <b className={n < concept.scores[i] ? 'on' : ''} key={n} />)}</i></div>)}</section><aside><button className={chosen ? 'chosen' : ''} onClick={toggle}>{chosen ? '★ In selezione' : '☆ Metti in selezione'}</button><button onClick={() => setFullscreen(!fullscreen)}>{fullscreen ? 'Riduci' : 'Schermo intero'} ↗</button></aside></div>
    ...
    <footer className="sv-lab-foot"><strong>La tua selezione</strong>{shortlist.length ? shortlist.map(id => { const x = CONCEPTS.find(c => c.id === id); return <button onClick={() => setActive(id)} key={id}>{x.n} · {x.name}</button>; }) : <span>Nessuna finalista selezionata.</span>}<small>Nessuna prova modifica lo Spazio Vivo pubblico.</small></footer>
  </section>;
}
